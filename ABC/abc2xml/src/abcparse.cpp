		       ...
		       char *comment)
{
	...
	s = (abcsym *)alloc_f(sizeof *s + client_sz);
	memset(s, 0, sizeof *s + client_sz);
	s->tune = t;
	if (text != 0) {
		s->text = (char *)alloc_f(strlen(text) + 1);
		strcpy(s->text, text);
	}
	if (comment != 0) {
		s->comment = (char *)alloc_f(strlen(comment) + 1);
			...
			struct abcsym *s)
{
	...
	nacc = s->u.key.nacc;
	for (;;) {
		if (nacc >= (signed int)sizeof (s->u.key.pits)) {
		       ...
		       char *scale)
{
	...
	p_clef->transpose = curvoice->add_pitch = transpose;
	p_clef->stafflines = -1;
	p_clef->staffscale = 0;
	if (lines != 0) {
		int l;
		      ...
		      struct abcsym *s)
{
	...
	if (clef_name != 0 || clef_middle != 0 || clef_lines != 0 || clef_scale != 0) {
		...
		s->type = ABC_T_CLEF;
		parse_clef(&s->u.clef, clef_name, clef_middle, clef_lines, clef_scale);
...
static char *get_line(void)
{
	...
	if (scratch_line == 0) {
		scratch_length = l + 1;
		scratch_line = (char *)malloc(scratch_length);
	}
	p = scratch_line;
	strncpy(p, line, l);
	p[l] = '\0';
	...
	if (*p == 'N' || *p == 'n')
		...
		for (;;) {
			while (isdigit((unsigned) *p)
			       && i < (signed int)sizeof s->u.meter.meter[0].top)
			...
			if (*p == '/') {
				...
				i = 0;
				while (isdigit((unsigned) *p)
				       && i < (signed int)sizeof s->u.meter.meter[0].bot)
...
/*fixme: should limit at 2 open bracket*/
		...
		default:
			if (!isalnum((unsigned) *p) && *p != '_')
				goto err;
			if (voice >= MAXVOICE) {
				syntax("Too many voices in %%staves", p);
				return;
			}
			{
				int v;
				...
				p_staff->voice = v;
				p_staff->name = (char *)alloc_f(strlen(voice_tb[v].name) + 1);
			 ...
			 struct abcsym *s)
{
	...
	if (*p == '"') {
		...
		while (*p != '"' && *p != '\0')
			p++;
		l = p - q;
		s->u.tempo.str1 = (char *)alloc_f(l + 1);
	...
	if (*p == 'C' || *p == 'c'
		...
		while (isdigit((unsigned) *p)) {
			...
			l = (BASE_LEN * top) / bot;
			if (l <= 0
			    || i >= (signed)sizeof s->u.tempo.length
					/ (signed)sizeof s->u.tempo.length[0])
				have_error++;
	...
	if (*p != '\0' && *p != '"') {
		...
		while (isspace((unsigned) p[-1]))
			p--;
		l = p - q;
		s->u.tempo.value = (char *)alloc_f(l + 1);
	...
	if (*p == '"') {
		...
		while (*p != '"' && *p != '\0')
			p++;
		l = p - q;
		s->u.tempo.str2 = (char *)alloc_f(l + 1);
		strncpy(s->u.tempo.str2, q, l);
		s->u.tempo.str2[l] = '\0';
	}

	return (char *)(have_error ? "Invalid tempo" : 0);
}
...
static struct kw_s {
	...
	s->u.voice.name = (char *)alloc_f(strlen(curvoice->name) + 1);
	...
	for (;;) {
		...
		case 0:			/* name */
			...
			s->u.voice.fname = (char *)alloc_f(strlen(name) + 1);
			strcpy(s->u.voice.fname, name);
			break;
		case 1:			/* subname */
			p = get_str(name, p, VOICE_NAME_SZ);
			s->u.voice.nname = (char *)alloc_f(strlen(name) + 1);
...
/*	curvoice->last_note = 0; */
	...
	if (bar_type != B_OBRA
		...
		s->text = (char *)alloc_f(strlen(repeat_value) + 1);
			      ...
			      int *stemless)
{
	...
	/* get the pitch */
	{
		...
		if (p_n == 0
		    || p_n - all_notes >= 14) {
			syntax( (char *)(acc ? "Missing note after accidental" : "Not a note"), p );
			pit = 16 + 7;	/* 'c' */
			if (*p == '\0')
				p--;
	...
	while (*p != '\0') {
		...
		if (gchord != 0) {
			if (is->text != 0) {
				...
				n = strlen(is->text);
				gch = (char *)alloc_f(n + strlen(gchord) + 2);
				strcpy(gch, is->text);
				gch[n] = '\n';
	...
	while (*p != '\0') {
		...
		case CHAR_GRACE:		/* '{' or '}' */
			if (p[-1] == '{') {
				...
				char_tb[(int)'{'] = CHAR_BAD;
				char_tb[(int)'}'] = CHAR_GRACE;
...
/*fixme:bad*/
		...
		case CHAR_SLASH:		/* '/' */
			...
			if (char_tb[(unsigned) *q] != CHAR_BAR)
				goto bad_char;
			syntax("Obsolete measure repeat syntax", p - 1);
			s = abc_new(t, 0, 0);
			s->type = ABC_T_MREP;
			s->state = ABC_S_TUNE;
			s->u.bar.type = 0;
...
/*fixme: KO if in grace note sequence*/
		...
		case CHAR_OPAR:			/* '(' */
			if (isdigit((unsigned) *p)) {
				...
				p = q;
				if (pplet < (signed)sizeof qtb / (signed)sizeof qtb[0])
					qplet = qtb[pplet];
				else	qplet = qtb[0];
				rplet = pplet;
				if (*p == ':') {
		...
		case CHAR_MINUS: {		/* '-' */
			...
			if (curvoice->dotted_tie) {
				curvoice->last_note->u.note.dotted_tie = 1;
			...
			char *p)
{
	...
	if (char_tb[(int)'{'] == CHAR_BAD) {	/* in a grace note sequence */
		s = abc_new(t, 0, 0);
		s->u.note.grace = 1;
	} else {
		s = abc_new(t, gchord, 0);
	...
	if (*p != 'Z' && !lyric_started && !s->u.note.grace) {
		lyric_started = 1;
