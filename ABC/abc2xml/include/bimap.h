...
#ifdef __MWERKS__ // this is for Code Warrior version 8.3
		multimap<T1,T2>::iterator insert (const pair<T1,T2>& x)  
			{ return exist (x) ? end() : multimap<T1,T2>::insert(x); }

		multimap<T1,T2>::iterator insert (multimap<T1,T2>::iterator position, const pair<T1,T2>& x)  
			{ return exist (x) ? end() : multimap<T1,T2>::insert(position, x); }
#else
